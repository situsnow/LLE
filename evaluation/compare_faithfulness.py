...
from utils.utils_functions import send_to_cuda, join_sentence, log_odds
from utils.constants import *

from utils.utils_data import handle_task_order, get_data_files, load_src_tgt
from utils.utils_algo import masked_top_weight_tokens, get_weights_from_explainer, masked_random_tokens, \
    prepare_load_fairseq_explainer
from utils.utils_model import prediction_by_bert, load_tokenizer_keyencoder, load_model
...
def set_parameters():
    ...
    parser.add_argument('--original-explainer', type=str, default=LRP, choices=[LIME, LRP])
    ...
    args.explainer_log_dir = os.path.join(args.dataset_path, args.explainer_log_dir,
                                          ...
                                          'auto' if args.original_explainer == LRP
...
def masked_explainer_tokens(src, weights, masked_token, args, interval, lrp=False):
    ...
    if lrp:
        ...
        # 2: neg;
        candidate_weights = [i for i in range(1, len(src) - 1) if weights[i-1] == 2]
        candidates = random.sample(candidate_weights, min(interval, len(candidate_weights)))
...
def main():
    ...
    for t in range(args.start_idx, args.end_idx):
        ...
        sq_explainer = prepare_load_fairseq_explainer(args, cur_config,
                                                      ...
                                                      retain_data_path=True)  # get dict.txt from upper level
        ...
        for split in splits:
            ...
            for i in range(len(src)):
                ...
                logodds_random_t.append(min(random_logodds))
